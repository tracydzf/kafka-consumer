...
func (monitor *Monitor) loopAcquireKafkaOffset() {
	...
	defer ticker.Stop()
	groupID := fmt.Sprintf("%s-%s", "SCHEDULER", monitor.topic)
	...
	for {
		...
		case <-monitor.Closer.Done():
			...
			if err != nil {
				log.Printf("get consume offset failed: topic[%s], pids[%+v]", monitor.topic, monitor.pids)
				continue
			}
...
func (monitor *Monitor) update(pid int32, consumerOffset int64) error {
	newestOffset, err := monitor.kafkaClient.GetOffset(monitor.topic, pid, sarama.OffsetNewest)
	if err != nil {
		log.Fatalf("get newest offset failed: topic[%s], pid[%d]", monitor.topic, pid)
		return err
	}
	...
	if err != nil {
		log.Fatalf("get oldest offset failed: topic[%s], pid[%d]", monitor.topic, pid)
		return err
	}
	monitor.oldestOffsetMap.setOffset(oldestOffset, pid)

	monitor.consumeOffsetMap.setOffset(consumerOffset, pid)
	return nil
}
